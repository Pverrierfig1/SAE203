...
foreach ($liste_util as $identifiant => $user) {  // $identifiant = clé (ex: jean.roland), $user = tableau des infos
    ...
    $role = htmlspecialchars(implode(", ", $user['roles'])); // Le rôles) est un tableau, on le transforme en chaîne
    // $bio = htmlspecialchars($user['bio']); // ligne en attente
